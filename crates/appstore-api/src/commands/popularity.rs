...
use crate::{
    ...
    output::Envelope,
    requests::{PopularityGroup, PopularityRequest},
};
...
pub async fn run(client: &ApiClient, args: &PopularityRequest) -> Result<Envelope> {
...
fn selected_countries(args: &PopularityRequest) -> Result<Vec<String>> {
...
mod tests {
    ...
    fn args(group: PopularityGroup, countries: Option<Vec<&str>>) -> PopularityRequest {
        PopularityRequest {
            app: "42".parse().unwrap(),
            group,
