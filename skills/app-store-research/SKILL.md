...
| Inspect displayed similar apps | `asapi similar 123456789 --country us` |
...
- Treat `iap` and `similar` separately for planning because each reads an `apps.apple.com` product page, but do not assume unlimited or independent capacity; Apple publishes no numeric limit or bucket relationship for product pages.
...
- Treat `similar.position` as the order in Apple's country-specific “You Might Also Like” shelf, not a universal similarity score or rank. `rating_count_display` is Apple's compact label (for example, `13M`), not an exact rating count.
